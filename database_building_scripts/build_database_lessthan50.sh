...
#Create all needed directories.
    SOM_OUT="$BASE_PATH/som_output_under50"
    if [[ ! -e $SOM_OUT ]]; then
        mkdir $SOM_OUT
    fi
    TRAINING_FILES="$BASE_PATH/training_under50"
    ...
    TRAINING_SHIFTED="$BASE_PATH/training_shifted_under50"
    if [[ ! -e $TRAINING_SHIFTED ]]; then
        mkdir $TRAINING_SHIFTED
    fi
    SOM_OUT_FILTERED="$BASE_PATH/som_output_filtered_under50"
    if [[ ! -e $SOM_OUT_FILTERED ]]; then
        mkdir $SOM_OUT_FILTERED
    fi
    SOM_OUT_SHIFTED="$BASE_PATH/som_output_shifted_under50"
    if [[ ! -e $SOM_OUT_SHIFTED ]]; then
        mkdir $SOM_OUT_SHIFTED
    fi
    SOM_OUT_FINAL="$BASE_PATH/som_output_final_under50"
    ...
    SHAPE_ANNOTATED="$BASE_PATH/anno_beds_under50"
    if [[ ! -e $SHAPE_ANNOTATED ]]; then
        mkdir $SHAPE_ANNOTATED
    fi
    SHAPE_ANNOTATED_SORTED="$BASE_PATH/anno_beds_sorted_under50"
    if [[ ! -e $SHAPE_ANNOTATED_SORTED ]]; then
        mkdir $SHAPE_ANNOTATED_SORTED
    fi
    SHAPE_ANNOTATED_FINAL="$BASE_PATH/anno_beds_final_under50"
    if [[ ! -e $SHAPE_ANNOTATED_FINAL ]]; then
        mkdir $SHAPE_ANNOTATED_FINAL
    fi
    CHROMHMM_INTERSECTS="$BASE_PATH/anno_intersects_under50"
    if [[ ! -e $CHROMHMM_INTERSECTS ]]; then
        mkdir $CHROMHMM_INTERSECTS
    fi
    DISTRIB_FIGS="$BASE_PATH/annotation_distribution_figs_under50"
    if [[ ! -e $DISTRIB_FIGS ]]; then
        mkdir $DISTRIB_FIGS
    fi
    PVALS="$BASE_PATH/pvalues_under50"
    if [[ ! -e $PVALS ]]; then
        mkdir $PVALS
    fi
    DATABASE_COMPREHENSIVE="$BASE_PATH/database_all_under50"
    if [[ ! -e ${DATABASE_COMPREHENSIVE}_${CELL_LINE} ]]; then
        mkdir ${DATABASE_COMPREHENSIVE}_${CELL_LINE}
    fi
    DATABASE="$BASE_PATH/database_under50"
    DATABASE_LOG="$BASE_PATH/database_log_under50"
