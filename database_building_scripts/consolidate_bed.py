...
def write_best_set(switch_vec, input_name, output_name):
    ...
    cluster_file = open(input_name + ".clust", "r")
    cluster_out = open(output_name + ".clust", "w")
    ...
    pos = -1
    [region_string, score] = get_next_region(input)
    [cluster_string, score] = get_next_region(cluster_file)
    ...
    for j in range(0, len(switch_vec)):
        ...
        while i < pos:
            ...
            if (pos - i) % 2 == 1 and not i == old_pos:
                ...
                cluster_out.write(cluster_string)
