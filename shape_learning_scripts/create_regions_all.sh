...
#Create all needed directories.
    ...
    split_and_shift() {
        factor=$1
        fac_fname=$(echo "${factor//./}")
        margin=$2
        if [[ ! -e $SPLIT_DIR/${fac_fname}_${margin}/ ]]; then
                mkdir $SPLIT_DIR/${fac_fname}_${margin}/
            fi
        if [[ ! -e $SPLIT_DIR/${fac_fname}_${margin}_shifted/ ]]; then
            mkdir $SPLIT_DIR/${fac_fname}_${margin}_shifted/
        fi
        if [[ ! -e $SPLIT_DIR/${fac_fname}_${margin}_crossings/ ]]; then
            mkdir $SPLIT_DIR/${fac_fname}_${margin}_crossings/
        fi
        if [[ ! -e $SPLIT_DIR/${fac_fname}_${margin}_percentile_cutoffs/ ]]; then
        mkdir $SPLIT_DIR/${fac_fname}_${margin}_percentile_cutoffs/
    fi
        for c in $CHROMS_NUM;
        do
            python ../common_scripts/split_regions.py ${WIG}/$c.wig $BIN_SIZE $c $SPLIT_DIR/${fac_fname}_${margin}/chrom${c}.pkl $REGION_SIZE $margin $factor 0.95 $SPLIT_DIR/${fac_fname}_${margin}_shifted/chrom${c}.pkl $SPLIT_DIR/${fac_fname}_${margin}_crossings/chrom${c}.txt $SPLIT_DIR/${fac_fname}_${margin}_percentile_cutoffs/chrom${c}.txt
