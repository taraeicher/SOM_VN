    ...
    do       
        ...
        do
            ...
            then
                ...
                python random_split.py ${training_files_peas[$i]}/${chrom}.pkl ${training_files_peas[$i]}/shifted_split_training_$repeat/${chrom}.pkl ${training_files_peas[$i]}/shifted_split_testing_$repeat/${chrom}.pkl
                ...
                python random_split.py ${perm_training_files_peas[$i]}/$repeat/${chrom}.pkl ${perm_training_files_peas[$i]}/shifted_split_training_$repeat/${chrom}.pkl ${perm_training_files_peas[$i]}/shifted_split_testing_$repeat/${chrom}.pkl
            ...
            do
                ...
                do
                    ...
                    qsub -A $PROJECT -m n -v TRAINING=$(realpath ${training_files[$i]}/shifted/${chrom}.pkl),BIN_SIZE=$BIN_SZ,CHROM=$chrom,BASE_PATH=$(realpath $BASE_PATH/${cell_types[$i]}_somvn_chromhmmperm/$chrom/$repeat/$alpha_0/$sigma_0/),GRID=$grid_size,CHROMHMM=$(realpath ${perm_chromhmm_anno[$i]}/${repeat}.bed),ITERATIONS=$epochs,LEARNING_RATE=$alpha_0,NEIGHBORHOOD=$sigma_0,REGION_SIZE=$REGION_SZ,SCRIPTS=$(realpath $SCRIPTS_DIR),CCCUTOFF=$CUTOFF,CUTOFFS=$(realpath ${training_files[$i]}/percentile_cutoffs/${chrom}.txt),IS_PEAS=False learn_shapes_for_chrom_som_vn_nogetopts.sh
                    ...
                    then
                        ...
                        qsub -A $PROJECT -m n -v TRAINING=$(realpath ${perm_training_files_peas[$i]}/$repeat/${chrom}.pkl),BIN_SIZE=$BIN_SZ,CHROM=$chrom,BASE_PATH=$(realpath $BASE_PATH/${cell_types[$i]}_somvn_peas_signalperm/$chrom/$repeat/$alpha_0/$sigma_0/),GRID=$grid_size,CHROMHMM=$(realpath ${ground_truth_peas[$i]}),ITERATIONS=$epochs,LEARNING_RATE=$alpha_0,NEIGHBORHOOD=$sigma_0,REGION_SIZE=$REGION_SZ,SCRIPTS=$(realpath $SCRIPTS_DIR),CCCUTOFF=$CUTOFF,CUTOFFS=$(realpath ${perm_training_files_peas[$i]}/$repeat/percentile_cutoffs/${chrom}.txt),IS_PEAS=True learn_shapes_for_chrom_som_vn_nogetopts.sh
