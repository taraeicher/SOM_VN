USAGE="This script is used for associating signal intensity with an RE using ChromHMM. This script will run the training on all 24 chromosomes. Parameters:\n
    ...
    BIN_SIZE=50
    TRAINING=""
    while getopts h:d:c:a:p:w: option; do
        case "${option}" in
...
do
    qsub -A $PROJECT learn_shapes_for_chrom_signal.sh -d $BASE_PATH -h $CHROMHMM -c $chrom -u $BASE_PATH/percentilecutoffs/$chrom.txt 
done
