...
do
    qsub -A $PROJECT learn_shapes_for_chrom_som.sh -d $BASE_PATH -h $CHROMHMM -t $CCCUTOFF -a -c $chrom -a $TRAINING -u $BASE_PATH/percentilecutoffs_som/$chrom.txt -s $SCRIPTS
done
