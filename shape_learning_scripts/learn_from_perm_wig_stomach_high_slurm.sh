#!/bin/bash
#SBATCH --job-name=learnPermShapesStomachHigh
#SBATCH --nodes=1 --ntasks=1 --mem=3g
#SBATCH --partition=gpu
#SBATCH --gres=gpu:k20x:1
#SBATCH --time=1:00:00
#SBATCH --output=shapesStomachHigh_%A_%a.out
#SBATCH --error=shapesStomachHigh_%A_%a.err
#SBATCH --array=1-22
...
#Variables
    ...
    CELL_LINE="stomach_high"
    REGION_SIZE=4000
    CHROMHMM=$BASE_PATH/chromhmm/stomach_E092.bed
    BIN_SIZE=50
	
#Create all needed directories.
    SOM_OUT=$BASE_PATH/som_${CELL_LINE}_perm
    if [[ ! -e $SOM_OUT ]]; then
        mkdir $SOM_OUT
    fi
    TRAINING_FILES=$BASE_PATH/training_${CELL_LINE}_perm
    if [[ ! -e $TRAINING_FILES ]]; then
        mkdir $TRAINING_FILES
    fi
    TRAINING_ANNOTATION_FILES=$BASE_PATH/training_annotation_${CELL_LINE}_perm
    if [[ ! -e $TRAINING_ANNOTATION_FILES ]]; then
        mkdir $TRAINING_ANNOTATION_FILES
    fi
    TRAINING_SHIFTED=$BASE_PATH/training_${CELL_LINE}_shifted_perm
    if [[ ! -e $TRAINING_SHIFTED ]]; then
        mkdir $TRAINING_SHIFTED
    fi
    SOM_OUT_FILTERED=$BASE_PATH/som_${CELL_LINE}_filtered_perm
    if [[ ! -e $SOM_OUT_FILTERED ]]; then
        mkdir $SOM_OUT_FILTERED
    fi
    SOM_OUT_SHIFTED=$BASE_PATH/som_${CELL_LINE}_shifted_perm
    if [[ ! -e $SOM_OUT_SHIFTED ]]; then
        mkdir $SOM_OUT_SHIFTED
    fi
    SOM_OUT_FINAL=$BASE_PATH/som_${CELL_LINE}_final_perm
    ...
    SHAPE_ANNOTATED=$BASE_PATH/anno_${CELL_LINE}_perm
    if [[ ! -e $SHAPE_ANNOTATED ]]; then
        mkdir $SHAPE_ANNOTATED
    fi
    SHAPE_ANNOTATED_SORTED=$BASE_PATH/anno_${CELL_LINE}_sorted_perm
    if [[ ! -e $SHAPE_ANNOTATED_SORTED ]]; then
        mkdir $SHAPE_ANNOTATED_SORTED
    fi
    SHAPE_ANNOTATED_FINAL=$BASE_PATH/anno_${CELL_LINE}_final_perm
    if [[ ! -e $SHAPE_ANNOTATED_FINAL ]]; then
        mkdir $SHAPE_ANNOTATED_FINAL
    fi
    CHROMHMM_INTERSECTS=$BASE_PATH/anno_${CELL_LINE}_intersect_perm
    if [[ ! -e $CHROMHMM_INTERSECTS ]]; then
        mkdir $CHROMHMM_INTERSECTS
    fi
	SHAPES_COMPREHENSIVE=$BASE_PATH/anno_${CELL_LINE}_consolidated_perm
    if [[ ! -e ${SHAPES_COMPREHENSIVE} ]]; then
        mkdir ${SHAPES_COMPREHENSIVE}
    fi
	
	#Permute WIG.
    python permute_wig.py $WIG_ORIG $WIG_PERM
	
	#Generate input files for training and annotation.
	./run_get_data $WIG_PERM $BIN_SIZE 0 Y ${SLURM_ARRAY_TASK_ID} $TRAINING_FILES/chrom${SLURM_ARRAY_TASK_ID} $REGION_SIZE
	./run_get_data $WIG_PERM $BIN_SIZE 0 N ${SLURM_ARRAY_TASK_ID} $TRAINING_ANNOTATION_FILES/chrom${SLURM_ARRAY_TASK_ID} $REGION_SIZE
