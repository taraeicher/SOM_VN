...
#Variables
    ...
    CHROMS_NUM="16 17 18 19 20 21 22"
    WIG_SPLIT_PATH=""
    BIN_SIZE=50
...
# Method for running the pipeline for a chromosome.
	run_pipeline() {
        ...
        awk '{ if ( $4 != "1_TssA" && $4 != "2_TssAFlnk" && $4 != "10_TssBiv" && $4 != "11_BivFlnk") { print; } }' $CHROMHMM/${CELL_LINE}_chromhmm_15_liftOver.bed > $CHROMHMM/${CELL_LINE}_chromhmm_nopromoter.bed
        awk '{ if ( $4 == "1_TssA" || $4 == "2_TssAFlnk" || $4 == "10_TssBiv" || $4 == "11_BivFlnk") { print; } }' $CHROMHMM/${CELL_LINE}_chromhmm_15_liftOver.bed > $CHROMHMM/${CELL_LINE}_chromhmm_onlypromoter.bed
         ...
         bedtools intersect -a $TRAINING_FILES/chrom${c}_nopromoter.bed -b $CHROMHMM/${CELL_LINE}_chromhmm_onlypromoter.bed -v > $TRAINING_FILES/chrom${c}_nopromoter_intersect.bed
         ...
         awk -F ',' '{printf ("chr%s\t%s\t%s\t%s\n", $1,$2,$3,$0)}' $TRAINING_ANNOTATION_FILES/chrom${c}window3 > $TRAINING_ANNOTATION_FILES/chrom${c}_nopromoter.bed
         bedtools intersect -a $TRAINING_ANNOTATION_FILES/chrom${c}_nopromoter.bed -b $CHROMHMM/${CELL_LINE}_chromhmm_onlypromoter.bed -v > $TRAINING_ANNOTATION_FILES/chrom${c}_nopromoter_intersect.bed
         ...
         #Save shapes to file.
         bedtools intersect -wao -a $SHAPE_ANNOTATED_FINAL/anno${c}.bed -b $CHROMHMM/${CELL_LINE}_chromhmm_nopromoter.bed > $CHROMHMM_INTERSECTS/anno${c}_nopromoter.bed			
         bedtools sort -i $CHROMHMM_INTERSECTS/anno${c}_nopromoter.bed > $CHROMHMM_INTERSECTS/anno${c}_sorted_nopromoter.bed
        python consolidate_chromHMM_nopromoter.py $CHROMHMM_INTERSECTS/anno${c}_sorted_nopromoter.bed $SOM_OUT_FINAL/chrom${c}som_centroid $SHAPES_COMPREHENSIVE ${WIG}/${CELL_LINE}.chr${c}.wig ${c} $CELL_LINE $TRAINING_ANNOTATION_FILES/chrom${c} 0
	}
