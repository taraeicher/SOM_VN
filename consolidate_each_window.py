...
def main():
	...
	#cluster_input_base = sys.argv[3]
	#cluster_output_base = sys.argv[4]
	num_window_sizes = 7
	...
	do_switch = [[] for x in range(0,len(sorted(glob.glob(input_base + "*"))))]
	
	#For each window size, find the optimal set of annotations.
	for i in range(0, num_window_sizes):
		build_best_set(do_switch[i], input_base + "map" + str(i))
	Parallel(n_jobs=len(sorted(glob.glob(input_base + "*"))))(delayed(write_best_set)(do_switch[i], input_base + "map" + str(i), output_base + "map" + str(i)) for i in range(0, len(sorted(glob.glob(input_base + "*")))))
...
def write_best_set(switch_vec, input_name, output_name):
	...
	pos = -1
	[region_string, score] = get_next_region(input)
	#cluster = get_next_cluster(cluster_file)
