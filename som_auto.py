...
def main():
	...
	if len(files) == num_window_sizes:
		...
		Parallel(n_jobs=num_window_sizes)(delayed(train_som)(window_sizes, files[i].name, i, som_centroids, som_centroid_counts, output_dir, intensity_threshold) for i in range(0, num_window_sizes))
		for file in files:
			file.close()
		
	#Print an error if number of files incorrect.
	else:
		print("Error: number of files in directory not equal to number of window sizes.")
...
"""
def train_som(window_sizes, file_name, win, som_centroids, som_centroid_counts, out_dir, intensity_threshold):
...
"""
def print_centroids(som_centroids, som_centroid_counts, win, out_dir, in_file_name, dim):

	#Print centroids.
	file = open(out_dir + "som_centroid" + str(win), "w")
	for i in range(len(som_centroids[win])):
		for j in range(len(som_centroids[win][i])):
			for k in range(len(som_centroids[win][i][j])):
				file.write(','.join(map(str, som_centroids[win][i][j][k])))
				file.write("\n")
	file.close()
	
	#Print centroid counts.
	file = open(out_dir + "som_centroid_counts" + str(win), "w")
	for i in range(len(som_centroid_counts[win])):
		for j in range(len(som_centroid_counts[win][i])):
			for k in range(len(som_centroid_counts[win][i][j])):
				file.write(str(som_centroid_counts[win][i][j][k]))
				file.write("\n")
	file.close()
