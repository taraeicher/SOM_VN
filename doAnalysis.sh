...
#Variables
	BASE_PATH="/fs/project/PAS0272/Tara/DNase_SOM/A549"
	...
	DIR="/fs/project/PAS0272/Tara/DNase_SOM/A549"
	
	
#Create all needed directories.
"""
	if [[ ! -e $BASE_PATH/som_output ]]; then
		mkdir $BASE_PATH/som_output
	fi
	if [[ ! -e $BASE_PATH/dataFiles ]]; then
		mkdir $BASE_PATH/dataFiles
	fi
	if [[ ! -e $BASE_PATH/dataFiles_anno ]]; then
		mkdir $BASE_PATH/dataFiles_anno
	fi
	if [[ ! -e $BASE_PATH/dataFiles_shifted ]]; then
		mkdir $BASE_PATH/dataFiles_shifted
	fi
	if [[ ! -e $BASE_PATH/som_output_filtered ]]; then
		mkdir $BASE_PATH/som_output_filtered
	fi
	if [[ ! -e $BASE_PATH/som_output_shifted ]]; then
		mkdir $BASE_PATH/som_output_shifted
	fi
	if [[ ! -e $BASE_PATH/som_output_final ]]; then
		mkdir $BASE_PATH/som_output_final
	fi
	if [[ ! -e $BASE_PATH/wig_chroms ]]; then
		mkdir $BASE_PATH/wig_chroms
	fi
	if [[ ! -e $BASE_PATH/anno_beds ]]; then
		mkdir $BASE_PATH/anno_beds
	fi
	if [[ ! -e $BASE_PATH/anno_beds_consolidated ]]; then
		mkdir $BASE_PATH/anno_beds_consolidated
	fi
	if [[ ! -e $BASE_PATH/anno_beds_final ]]; then
		mkdir $BASE_PATH/anno_beds_final
	fi
"""
#FASTQ alignment and processing.
"""
	echo -e "---------------------------------------Aligning to reference genome at $REF_GENOME.-----------------------------------"
    bowtie2 -x $REF_GENOME -U $FASTQ_DIR/$BASE_FILENAME.fastq -k 1  -S $BASE_PATH/${BASE_FILENAME}_trimmed.bam --threads 16 --trim3 16 
...
#Output RPKM intensities in a WIG file.
	...
	gosr binbam -f 0 -n 1000 -t $BASE_FILENAME $BASE_PATH/$BASE_FILENAME.bam $BIN_SIZE $BASE_FILENAME > $BASE_PATH/$BASE_FILENAME.wig
	echo -e "------------------------------------------------------WIG file complete.-------------------------------------------------\n"
	
#Split WIG files by chromosome.
	echo -e "\n---------------------------------------------Splitting WIG file by chromosome.-------------------------------------------"
	python $WIG_SPLIT_PATH/wig_split.py $BASE_PATH/$BASE_FILENAME.wig $BASE_PATH/wig_chroms/$BASE_FILENAME
	if [[ -e $BASE_PATH/wig_chroms/$BASE_FILENAME/*chrM* ]]; then
		rm $BASE_PATH/wig_chroms/$BASE_FILENAME/*chrM*
	fi
	if [[ -e $BASE_PATH/wig_chroms/$BASE_FILENAME/*random* ]]; then
		rm $BASE_PATH/wig_chroms/$BASE_FILENAME/*random*
	fi
	if [[ -e $BASE_PATH/wig_chroms/$BASE_FILENAME/*chrM* ]]; then
		rm $BASE_PATH/wig_chroms/$BASE_FILENAME/*chrM*
	fi
	if [[ -e $BASE_PATH/wig_chroms/$BASE_FILENAME/*_g* ]]; then
		rm $BASE_PATH/wig_chroms/$BASE_FILENAME/*_g*
	fi
...
#Method for running the pipeline for a chromosome.
	run_pipeline() {
		...
		./runGetData $BASE_PATH/wig_chroms/$BASE_FILENAME.chr$c.wig $BIN_SIZE 0 Y $c $BASE_PATH/dataFiles/chrom$c
		./runGetData $BASE_PATH/wig_chroms/$BASE_FILENAME.chr$c.wig $BIN_SIZE 0 N $c $BASE_PATH/dataFiles_anno/chrom$c
		echo -e "-------------------------------------Data formatting complete for chrom $c.------------------------------------------------\n"
		
		#Shuffle the input files.
		echo -e "\n--------------------------------------Shuffling the training data for chrom $c.--------------------------------------------"
		cd $BASE_PATH/dataFiles/
		window="window"
		...
		python shift_input.py $BASE_PATH/dataFiles/shuf_chrom$c$window $BASE_PATH/dataFiles_shifted/chrom$c$window $BIN_SIZE $BASE_PATH/wig_chroms/$BASE_FILENAME.chr$c.wig
		echo -e "----------------------------------------------Shifting complete for chrom $c.-------------------------------------------------\n"
		
		#Run the SOM.
		echo -e "\n--------------------------------Building the SOM model for chrom $c. This may take a while.-----------------------------------"
		module load cuda/$CUDA_VERSION
		python som_auto.py $BASE_PATH/dataFiles_shifted/chrom$c $BASE_PATH/som_output/chrom$c $BASE_PATH/$BASE_FILENAME.wig
		echo -e "---------------------------------------------SOM model is ready for chrom $c.---------------------------------------------------\n"
		
		#Remove all SOM centroids to which no regions map.
		echo -e "\n----------------------------------------Removing unused SOM centroids $c.--------------------------------------------"
		python remove_by_cutoff.py $BASE_PATH/som_output/som_centroid_chrom$c $WINDOW_SIZE_COUNT 1 $BASE_PATH/som_output_filtered/som_centroid_chrom$c
		echo -e "------------------------------------------------Removal complete for chrom $c.-----------------------------------------------------\n"
		
		#Merge shifted regions.
		echo -e "\n----------------------------------------Merging shifted centroids for chrom $c.--------------------------------------------------"
		python merge_shifted.py $WINDOW_SIZE_COUNT $BASE_PATH/som_output_filtered/som_centroid_chrom$c $BASE_PATH/som_output_shifted/som_centroid_chrom$c
		...
		python kmeans_centroids.py $WINDOW_SIZE_COUNT $BASE_PATH/som_output_shifted/som_centroid_chrom$c $BASE_PATH/som_output_final/som_centroid_chrom$c
		echo -e "-------------------------------------------------K-means complete for chrom $c.---------------------------------------------------\n"
		
		#Annotate regions with cluster data.
		echo -e "\n------------------------------------Annotating the regions by cluster for chrom $c.----------------------------------------------"
		python make_cluster_bed.py $BASE_PATH/dataFiles_anno/chrom$c$window $BASE_PATH/som_output_final/som_centroid_chrom$c $BASE_PATH/anno_beds/anno$c
		...
		if test "$(ls -A "$BASE_PATH/anno_beds/")"; then
			for bedfile in $BASE_PATH/anno_beds/*;
				do
					pos=$(echo $bedfile | grep -b -o anno_beds/ | awk 'BEGIN {FS=":"}{print $1}')
					after=${bedfile:pos}
					pos=$pos+10
					after=$(echo ${bedfile:$pos})
					bedtools sort -i  $BASE_PATH/anno_beds/$after > ~/out/A549/anno_beds/sorted_$after
				done
		fi
		python consolidate_each_window.py $BASE_PATH/anno_beds/anno$c $BASE_PATH/anno_beds_consolidated/anno$c
