		...
		for(i = 0; i < args->numWindowSizes; i++)
		{
			...
			for(steps = 0; steps < stepsCount; steps++)
			{
				...
				while(atEnd != NULL)
				{
					...
					if(gapExists == 0)
					{
						//Check to see if the region contains anything above the lower cutoff.
						//If not, we don't need it.
                        //Also exclude any region with any value above the max cutoff.
						int belowCutoff = 1;
                        int aboveCutoff = 0;
						int sig;
						for(sig = 0; sig < windowSize / args->resolution; sig++)
						{
                            ...
                            if(signalList[sig] > MAX_RPKM)
							{
								aboveCutoff = 1;
							}
						}
						if(belowCutoff == 0 && aboveCutoff == 0)
						{
