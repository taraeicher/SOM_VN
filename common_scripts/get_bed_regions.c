...
{	
	...
	{		
        ...
        while(atEnd_bed != NULL && atEnd != NULL)
        {
            ...
            if(intervalStart >= prevEnd && intervalEnd - intervalStart >= windowSize)
            {  
                //If the region is larger than the window size, cut it.
                if(intervalEnd - intervalStart > windowSize)
                {
