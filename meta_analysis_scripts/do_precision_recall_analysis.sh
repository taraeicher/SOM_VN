...
for i in 0 1 2 3 4 5 6 7 8;
    do
        ...
        python plot_precision_recall.py $ANNO_MERGED/ $TSS_MERGED/ $ANNO_MERGED_RPKM/ $ANNO_MERGED_PERM_SUB/ $ANNOTATED/ $TSS_PROMOTER_CLUST/ $ANNOTATED_WITH_TSS/ $ANNOTATED_TSS_AND/ $ANNOTATED_RPKM/ $ANNOTATED_PERM/ $BASE/  $PRECISION_RECALL/ $UNKNOWNS/ $WIG $WINDOW_INDEX ${dest_cell} ${src_cell} 0
        
        if [ $i = 4 ]; then
            python plot_precision_recall.py $ANNO_MERGED_SUB/ $TSS_MERGED_SUB/ $ANNO_MERGED_RPKM/ $ANNO_MERGED_PERM/ $ANNOTATED_SUB/ $TSS_PROMOTER_CLUST/ $ANNOTATED_WITH_TSS_SUB/ $ANNOTATED_TSS_AND_SUB/ $ANNOTATED_RPKM/ $ANNOTATED_PERM/ $BASE/sub  $PRECISION_RECALL_SUB/ $UNKNOWNS_SUB/ $WIG $WINDOW_INDEX ${dest_cell} ${src_cell} 1
        fi
