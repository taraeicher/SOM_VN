...
def main():
    ...
    cell = sys.argv[6]
    src = sys.argv[7]
    cutoffs = np.genfromtxt(sys.argv[8], dtype = str)
    ...
    precision_avg = np.zeros((len(cutoffs), 3))
    recall_avg = np.zeros((len(cutoffs), 3))
    for j in range(len(cutoffs)):
        cutoff = cutoffs[j]
        c = 0
        for chrom in all_chroms:
            #Get files for each category.
            our_bed = sys.argv[1] + "anno" + str(chrom) + "_" + str(cutoff) + ".bed"
            our_sig = sys.argv[2] + "clusters_anno" + str(chrom) + "_" + str(cutoff)
            wig = sys.argv[5] + str(chrom) + ".wig"
            
            #Get all precision and recall values.
            [precision, recall, threshold, predictions, ground_truth, fpr] = get_all_precision_and_recall(our_bed, our_sig, wig, chrom, cell)
            ...
            print_report(precision, recall, chrom, cell, pr_path, fpr, cutoff)
        
        #Average across chromosomes.
        precision_avg[j,:] = np.mean(precision_all, axis = 1)
        recall_avg[j,:] = np.mean(recall_all, axis = 1)
        
    #Save averaged plot.
    save_plot(precision_avg, recall_avg, plot_out, cell, src)
...
def get_all_precision_and_recall(bed, sig, wig, chrom, cell):
    ...
    return [precision, recall, threshold, pred, gt, fpr]
    
"""
Print precision and recall for all chromosomes.
""" 
def print_report(precision, recall, chrom, cell, pr, fpr, cutoff):
    #Print the cell line, chromosome, and window information
    report = open(pr + "report_" + str(cutoff) + "_" + chrom, "w")
    fpr_report = open(pr + "fpr_report_" + str(cutoff) + "_" + chrom, "w")
    report.write(cell + "\n")
...
"""
Save the precision-recall curve depending on the percentile cutoff for RE association.
"""
def save_plot(our_precision, our_recall, out, cell, src):
    ...
    #Plot our data.
    gr_0 = (our_precision[:,0] > 0) | (our_recall[:,0] > 0)
    plt.plot(our_precision[gr_0,0], our_recall[gr_0,0], marker = None, color = promoter_color, linestyle = promoter_style)
    gr_1 = (our_precision[:,1] > 0) | (our_recall[:,1] > 0)
    plt.plot(our_precision[gr_1,1], our_recall[gr_1,1], marker = None, color = enhancer_color, linestyle = enhancer_style)
    gr_2 = (our_precision[:,2] > 0) | (our_recall[:,2] > 0)
    plt.plot(our_precision[gr_2,2], our_recall[gr_2,2], marker = None, color = weak_color, linestyle = weak_style)
        
    #Save the plot.
    print(our_precision)
    print(our_recall)
    plt.savefig(out + "preicion_recall.png")
    plt.close()
    
    #Add the legend.
    legend_elements = [Line2D([0], [0], marker=None, label='Promoter', color = promoter_color, linestyle = promoter_style),
                        Line2D([0], [0], marker=None, label='Enhancer', color = enhancer_color, linestyle = enhancer_style),
                        Line2D([0], [0], marker=None, label='Weak', color = weak_color, linestyle = weak_style)]
    plt.legend(handles=legend_elements, loc="lower right")
    
    #Save the plot.
    plt.savefig(out + "precision_recall_legend.png")
    plt.close()
