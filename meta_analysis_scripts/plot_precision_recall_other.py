...
def get_all_precision_and_recall(bed, sig, tss_bed, tss_sig, or_bed, or_sig, and_bed, and_sig, rpkm_bed, rpkm_sig, wig, chrom, win, cell):
    ...
    annotations = ["Promoter", "Enhancer", "Weak", "Other"]
    ...
    pred_reduced = pred[np.where(gt[:,3] == 0),:][0,:]
    gt_reduced = gt[np.where(gt[:,3] == 0),:][0,:]
    precision = dict()
    recall = dict()
    for i in range(length):
        precision[i] = precision_score(gt_reduced[:, i], pred_reduced[:, i])
        recall[i] = recall_score(gt_reduced[:, i], pred_reduced[:, i])
    ...
    pred_or_reduced = pred_or[np.where(gt_or[:,3] == 0),:][0,:]
    gt_or_reduced = gt_or[np.where(gt_or[:,3] == 0),:][0,:]
    precision_or = dict()
    recall_or = dict()
    for i in range(length):
        precision_or[i] = precision_score(gt_or_reduced[:, i], pred_or_reduced[:, i])
        recall_or[i] = recall_score(gt_or_reduced[:, i], pred_or_reduced[:, i])
    ...
    pred_and_reduced = pred_and[np.where(gt_and[:,3] == 0),:][0,:]
    gt_and_reduced = gt_and[np.where(gt_and[:,3] == 0),:][0,:]
    precision_and = dict()
    recall_and = dict()
    for i in range(length):
        precision_and[i] = precision_score(gt_and_reduced[:, i], pred_and_reduced[:, i])
        recall_and[i] = recall_score(gt_and_reduced[:, i], pred_and_reduced[:, i])
    ...
    pred_rpkm_reduced = pred_rpkm[np.where(gt_rpkm[:,3] == 0),:][0,:]
    gt_rpkm_reduced = gt_rpkm[np.where(gt_rpkm[:,3] == 0),:][0,:]
    precision_rpkm = dict()
    recall_rpkm = dict()
    for i in range(length):
        precision_rpkm[i] = precision_score(gt_rpkm_reduced[:, i], pred_rpkm_reduced[:, i])
        recall_rpkm[i] = recall_score(gt_rpkm_reduced[:, i], pred_rpkm_reduced[:, i])
...
def save_scatterplot(our_precision, our_recall, tss_precision, tss_recall, or_precision, or_recall, and_precision, and_recall, rpkm_precision, rpkm_recall, out, cell, src, indices_to_highlight, chroms, separate_legend, make_big):
    ...
    if make_big:
        ...
        #Plot our data.
        plt.scatter(our_precision[0,:], our_recall[0,:], c = promoter_color, marker = our_symbol, edgecolor = "black")
        plt.scatter(our_precision[1,:], our_recall[1,:], c = enhancer_color, marker = our_symbol, edgecolor = "black")
        plt.scatter(our_precision[2,:], our_recall[2,:], c = weak_color, marker = our_symbol, edgecolor = "black")
    ...
    try:
        ...
        for i in range(0, bed.shape[0]):            
            ...
            if next_start != current_start and not_annotated_count != count_in_region:
                ...
                elif sum_vec[0] == 0 and sum_vec[1] == 0 and sum_vec[2] == 0 and sum_vec[3] == 0:
                    del vec_pred[len(vec_pred) - 1]
                    del vec_gt[len(vec_pred) - 1]
...
def save_misprediction_heatmap(predictions, ground_truth, path, cell, src, enhancer):
    ...
    if src == "H1" and enhancer:
        ...
        mispredictions = np.stack([promoter_pred_vec[0:3], weak_pred_vec[0:3]])
    else:
        ...
        weaks = predictions[where_weaks]
        weak_pred_vec = np.sum(weaks, axis = 0) / weaks.shape[0]
        mispredictions = np.stack([promoter_pred_vec[0:3], enhancer_pred_vec[0:3], weak_pred_vec[0:3]])
    ...
    if src == "H1" and enhancer:
        where_pred_weaks = np.where(predictions[:,1] == 1)[0]
        gt_weaks = ground_truth[where_pred_weaks]
