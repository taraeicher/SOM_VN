...
#Create all needed directories.
    ...
    DATABASE="$BASE_FILENAME/Brain/chromHmm_merged2"
    ...
    ANNOTATED="$BASE_FILENAME/$CELL_LINE/annotated_Brain_sub_tss_and"
    if [[ ! -e $ANNOTATED ]]; then
		mkdir $ANNOTATED
	fi
    ANNOTATED_SORTED="$BASE_FILENAME/$CELL_LINE/annotated_Brain_sub_sorted_tss_and"
    if [[ ! -e $ANNOTATED_SORTED ]]; then
		mkdir $ANNOTATED_SORTED
	fi
    ANNOTATED_CONSOLIDATED="$BASE_FILENAME/$CELL_LINE/annotated_Brain_sub_consolidated_tss_and"
