...
#Create all needed directories.
    ...
    DATABASE="$BASE_FILENAME/Brain/database_brain3"
    ...
    ANNOTATED="$BASE_FILENAME/$CELL_LINE/annotated_brain3"
    if [[ ! -e $ANNOTATED ]]; then
		mkdir $ANNOTATED
	fi
    ANNOTATED_SORTED="$BASE_FILENAME/$CELL_LINE/annotated_sorted_brain3"
    if [[ ! -e $ANNOTATED_SORTED ]]; then
		mkdir $ANNOTATED_SORTED
	fi
    ANNOTATED_CONSOLIDATED="$BASE_FILENAME/$CELL_LINE/annotated_consolidated_brain3"
