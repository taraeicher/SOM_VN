...
def main():
    ...
    match_clusters_cutoff(in_file.name, database_file.name, output_dir, wig_path, cutoff)
    in_file.close()
    database_file.close()
...
"""
def match_clusters_cutoff(in_file_name, cluster_file_name, out_dir, wig_name, cutoff):
    ...
    if(len(clusters) > 1):
        ...
        while(next_line):
            ...
            [match, ambig, crosscorr, out_str] = match_datum(input_scaled, clusters, cluster_anno)
...
"""
def match_datum(datum, clusters, annotations):
