#!/bin/bash
#SBATCH --job-name=annotateStomachLowFromHigh
#SBATCH --nodes=1 --ntasks=1 --mem=3g
#SBATCH --time=1:00:00
#SBATCH --output=annotateStomachLowFromHigh_%A_%a.out
#SBATCH --error=annotateStomachLowFromHigh_%A_%a.err
#SBATCH --array=1-22
...
CELL_LINE_SRC="stomach_high"
CELL_LINE_TGT="stomach_low"
TO_ANNOTATE=$BASE_PATH/training_annotation_${CELL_LINE_TGT}
SHAPES=$BASE_PATH/shapes_${CELL_LINE_SRC}
CHROMHMM=$BASE_PATH/chromhmm/stomach_E092.bed
WIG=$BASE_PATH/wig/${CELL_LINE_TGT}
ANNOTATED_TGT="$BASE_PATH/annotated_${CELL_LINE_SRC}_${CELL_LINE_TGT}"
    if [[ ! -e $ANNOTATED_TGT ]]; then
        mkdir $ANNOTATED_TGT
    fi
ANNOTATED_SORTED_TGT="$BASE_PATH/annotated_sorted_${CELL_LINE_SRC}_${CELL_LINE_TGT}"
    if [[ ! -e $ANNOTATED_SORTED_TGT ]]; then
        mkdir $ANNOTATED_SORTED_TGT
    fi
ANNOTATED_CONSOLIDATED_TGT="$BASE_PATH/annotated_consolidated_${CELL_LINE_SRC}_${CELL_LINE_TGT}"
    if [[ ! -e $ANNOTATED_CONSOLIDATED_TGT ]]; then
        mkdir $ANNOTATED_CONSOLIDATED_TGT
    fi
ANNO_MERGED="$BASE_PATH/annotated_merged_${CELL_LINE_SRC}_${CELL_LINE_TGT}"
        if [[ ! -e $ANNO_MERGED ]]; then
            mkdir $ANNO_MERGED
        fi
    
#Annotating the regions.
python make_annotated_bed.py $TO_ANNOTATE/chrom${SLURM_ARRAY_TASK_ID} $SHAPES $ANNOTATED_TGT/anno${SLURM_ARRAY_TASK_ID} $WIG/${CELL_LINE_TGT}.chr${SLURM_ARRAY_TASK_ID}.wig  0.0
